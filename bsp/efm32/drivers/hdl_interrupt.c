...
void UART0_TX_IRQHandler(void)
{
	...
	{
		/* invoke callback function */
		if (uartCbTable[0].cbFunc != RT_NULL)
		{
			(uartCbTable[0].cbFunc)(uartCbTable[0].userPtr);
		}
...
void UART0_RX_IRQHandler(void)
{
	...
	{
		/* invoke callback function */
		if (uartCbTable[1].cbFunc != RT_NULL)
		{
			(uartCbTable[1].cbFunc)(uartCbTable[1].userPtr);
		}
	}
}
#endif
...
void UART1_TX_IRQHandler(void)
{
	...
	{
		/* invoke callback function */
		if (uartCbTable[2].cbFunc != RT_NULL)
		{
			(uartCbTable[2].cbFunc)(uartCbTable[2].userPtr);
		}
...
void UART1_RX_IRQHandler(void)
{
	...
	{
		/* invoke callback function */
		if (uartCbTable[3].cbFunc != RT_NULL)
		{
			(uartCbTable[3].cbFunc)(uartCbTable[3].userPtr);
		}
	}
}
#endif
...
#endif
#if defined(UART_PRESENT)
	case efm32_irq_type_uart:
		uartCbTable[hook->unit].cbFunc = hook->cbFunc;
		uartCbTable[hook->unit].userPtr = hook->userPtr;
		break;
#endif
#if defined(LEUART_PRESENT)
