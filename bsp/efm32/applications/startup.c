...
void rtthread_startup(void)
{
...
#if defined(RT_USING_EBI)
    rt_hw_ebi_init();
#endif

#ifdef RT_USING_HEAP
    #if defined(RT_USING_EXT_SRAM) && defined(RT_USING_MEMHEAP_AS_HEAP)
        rt_system_heap_init((void*)EFM32_EXT_HEAP_BEGIN, (void*)EFM32_EXT_HEAP_END);
        rt_memheap_init(&ext_heap, "system", (void*)&__bss_end__, (rt_uint32_t)EFM32_SRAM_END - (rt_uint32_t)&__bss_end__ - 64);
    #else
        #ifdef __CC_ARM
    ...
    /* enable interrupt */
    //rt_hw_interrupt_enable(0x0UL); //有问题，启用bootloader引导式，会异常
